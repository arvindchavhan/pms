...
public class PaymentController {
    ...
    @PostMapping("/create")
    public ResponseEntity<ResponseDTO<CreatePaymentDto>> createPayment(CreatePaymentRequest request) {
        LOGGER.info("request : " + request);

        ResponseDTO<CreatePaymentDto> responseDTO = new ResponseDTO<>();

        try {
            CompletableFuture<CreatePaymentDto> response = paymentService.createPayment(request);
            CreatePaymentDto dto = response.get();
            if (dto == null) {
